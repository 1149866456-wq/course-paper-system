...
def submit():
    ...
    if existing:
        ...
        old_file_path = f"{course_name}/{old_filename}"
...
def admin_course(course_id):
    ...
    submissions_response = supabase.table("submissions").select("*, students(name, student_id)").eq("course_id", course_id).execute()
...
def admin_download(course_id):
    ...
    with zipfile.ZipFile(memory_file, "w", zipfile.ZIP_DEFLATED) as zf:
        for sub in submissions:
            ...
            file_path = f"{course_name}/{sub['filename']}"
            try:
